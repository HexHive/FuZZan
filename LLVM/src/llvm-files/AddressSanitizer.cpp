...
#define ENABLERBTREE 2
#define ENABLEMINSHADOW 3
#define ENABLESAMPLE 4

int isFuZZanEnabled() {
  char *env = getenv("FUZZAN_MODE");
  int FuZZanMode = 1;
  if (env)
    FuZZanMode = atoi(env);

  // native mode
  if (FuZZanMode == 1 || FuZZanMode == 2)
    return 0;

  if (FuZZanMode == 3)
    return ENABLERBTREE;

  if (FuZZanMode == 4 || FuZZanMode == 5 || FuZZanMode == 6 || FuZZanMode == 7)
    return ENABLEMINSHADOW;
...
static uint64_t kAsanCtorAndDtorPriority = 1;
...
public:
  FuZZanInit() {
    ...
    if (env)
      FuZZanMode = atoi(env);
    if (FuZZanMode >= 2 && FuZZanMode <= 8)
      kAsanCtorAndDtorPriority = 0;
    else
      kAsanCtorAndDtorPriority = 1;
  }
};

class FuZZanInit FInit;
...
static cl::opt<bool> ASanInlineOpt(
    "asan-inline-opt",
    cl::desc("Apply inline optimization for safety check"),
    cl::Hidden, cl::init(false));
...
private:
  ...
  GlobalVariable *ASanHashMapCache;
  ...
  Function *AsanSafetyCheck;
  Function *AsanStackInsert;
  Function *AsanStackDelete;
  Function *AsanStackDeleteRBtreeAdd;
  Function *AsanPtrCmpFunction, *AsanPtrSubFunction;
  Constant *AsanShadowGlobal;

  Function *SampleSafetyCheck;
  Function *SampleStackInsert;
  Function *SampleStackDelete;
...
Value *AddressSanitizer::memToShadow(Value *Shadow, IRBuilder<> &IRB) {
  ...
  if ((isFuZZanEnabled() == ENABLEMINSHADOW) || (isFuZZanEnabled() == ENABLESAMPLE)) {
...
void AddressSanitizer::instrumentAddress(Instruction *OrigIns,
                                         ...
                                         Value *SizeArgument, bool UseCalls,
                                         uint32_t Exp) {
  if (isFuZZanEnabled() == ENABLERBTREE) {
      IRBuilder<> IRB(InsertBefore);
      Value *AddrLong = IRB.CreatePointerCast(Addr, IntptrTy);
      Value *Size = ConstantInt::get(IRB.getInt32Ty(), TypeSize / 8);
      if (AddrLong)
        IRB.CreateCall(AsanSafetyCheck, {AddrLong, Size});
  } else {
    bool IsMyriad = TargetTriple.getVendor() == llvm::Triple::Myriad;
    ...
    if (isFuZZanEnabled() == ENABLESAMPLE)
      IRB.CreateCall(SampleSafetyCheck);
...
void AddressSanitizer::initializeCallbacks(Module &M) {
  ...
  if ((isFuZZanEnabled() == ENABLERBTREE)) {
    AsanSafetyCheck = checkSanitizerInterfaceFunction(M.getOrInsertFunction(
        "__asan_safety_check", IRB.getVoidTy(), IntptrTy, IRB.getInt32Ty()));
    AsanStackInsert = checkSanitizerInterfaceFunction(M.getOrInsertFunction(
        "__asan_stack_hexasan_insert", IRB.getVoidTy(), IntptrTy, IntptrTy));
    AsanStackDelete = checkSanitizerInterfaceFunction(M.getOrInsertFunction(
        "__asan_stack_hexasan_delete", IRB.getVoidTy(), IntptrTy, IntptrTy));
    AsanStackDeleteRBtreeAdd = checkSanitizerInterfaceFunction(M.getOrInsertFunction(
        "__asan_stack_hexasan_delete_rbtree_add", IRB.getVoidTy(), IntptrTy, IntptrTy));
  }

  if (isFuZZanEnabled() == ENABLESAMPLE) {
    SampleSafetyCheck = checkSanitizerInterfaceFunction(M.getOrInsertFunction(
        "__sample_safety_check", IRB.getVoidTy()));
    SampleStackInsert = checkSanitizerInterfaceFunction(M.getOrInsertFunction(
        "__sample_stack_insert", IRB.getVoidTy()));
    SampleStackDelete = checkSanitizerInterfaceFunction(M.getOrInsertFunction(
        "__sample_stack_delete", IRB.getVoidTy()));
  }
}
...
if (isFuZZanEnabled() == ENABLERBTREE) {
  IRB.CreateCall(ASan.AsanStackInsert,
                 {LocalStackBase, ConstantInt::get(IntptrTy, ShadowAfterScope.size() * L.Granularity)});
  ...
  for (const auto &Desc : SVD) {
    ...
    if (!islocated) {
      Value *RedzonePtr = IRB.CreateIntToPtr(
        IRB.CreateAdd(LocalStackBase, ConstantInt::get(IntptrTy, Desc.Offset)),
        Desc.AI->getType());
      IRB.CreateCall(ASan.AsanStackDeleteRBtreeAdd,
                     {RedzonePtr,
                     ConstantInt::get(IntptrTy, Desc.Size)});
    }
  }
  ...
  if (!StaticAllocaPoisonCallVec.empty()) {
    ...
    for (const auto &APC : StaticAllocaPoisonCallVec) {
      ...
      size_t Begin = Desc.Offset;
      size_t End = Begin + APC.Size;
      Value *RedzonePtr = IRB.CreateIntToPtr(
              IRB.CreateAdd(LocalStackBase, ConstantInt::get(IntptrTy, Begin)),
              APC.AI->getType());
      IRBuilder<> IRB(APC.InsBefore);
      if (APC.DoPoison) {
        IRB.CreateCall(ASan.AsanStackInsert,
                       {RedzonePtr, ConstantInt::get(IntptrTy, End - Begin)});
      }
      else {
      IRB.CreateCall(ASan.AsanStackDelete,
                     {RedzonePtr, ConstantInt::get(IntptrTy, End - Begin)});
      }
    }
  }
  ...
  for (auto Ret : RetVec) {
    ...
    IRBRet.CreateCall(ASan.AsanStackDelete,
                      {LocalStackBase, ConstantInt::get(IntptrTy,  L.FrameSize)});
  }
} else {
  if (isFuZZanEnabled() == ENABLESAMPLE) {
    IRB.CreateCall(ASan.SampleStackInsert);
    ...
    if (!StaticAllocaPoisonCallVec.empty()) {
      ...
      for (const auto &APC : StaticAllocaPoisonCallVec) {
        ...
        if (APC.DoPoison)
          ...
          IRB.CreateCall(ASan.SampleStackDelete);
      }
    }
    ...
    for (auto Ret : RetVec) {
      IRBuilder<> IRBRet(Ret);
      IRBRet.CreateCall(ASan.SampleStackDelete,
                        {LocalStackBase, ConstantInt::get(IntptrTy,  L.FrameSize)});
    }
  }
